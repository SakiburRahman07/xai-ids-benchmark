"""Generate notebooks/run_in_colab.ipynb (avoids hand-escaping JSON)."""
import json
...
md(["## 2. Clone the repo and install dependencies"])
code([
...
code([
    ...
    "import pkgutil\n",
    "def have(name): return pkgutil.find_loader(name) is not None\n",
    "pkgs = ['numpy','pandas','scipy','scikit-learn','pyyaml','tqdm','joblib',\n",
    "        'shap','lime','dice_ml','captum','rtdl','matplotlib','seaborn','requests','statsmodels']\n",
...
out = Path(__file__).resolve().parent.parent / "notebooks" / "run_in_colab.ipynb"
